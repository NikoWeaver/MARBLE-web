// Classic (non-module) script on purpose: some hosts serve the page in a CSP
// sandbox without allow-same-origin, where same-host module scripts and
// fetch()/XHR to the site's own files fail. three.js still comes from the CDN
// via the import map (the CDN sends CORS headers), and the model data is
// loaded with plain <script src> tags generated by tools/bake_model.py.
(async function initRobotViewer() {
  // Updated by tools/bake_model.py whenever the baked model data changes.
  const MODEL_DATA_VERSION = "3e10073a07";
  ...
  try {
    ...
    if (loadingEl) loadingEl.innerHTML = "<p style='color:#e0662b'>Failed to load 3D library</p>";
    return;
  }
  ...
  // --- Load baked assembly + mesh data (classic <script> loads, no fetch/XHR) ---
  ...
  // Decode one baked mesh (quantized, welded, indexed) into the same
  // non-indexed, flat-normal BufferGeometry the STL loader used to produce.
  function decodeMesh(rec) {
  ...
  function registerChunk(chunk) {
    for (const rec of chunk.meshes) {
      ...
      for (const name of rec.names) geometryCache.set(name, geo);
    }
  }
  ...
  async function loadAssembly() {
    try {
      const setProgress = (msg) => {
        const span = loadingEl && loadingEl.querySelector("span");
        if (span) span.textContent = msg;
      };

      if (!window.MARBLE_ASSEMBLY) {
        await loadScript(`media/models/assembly-data.js?v=${MODEL_DATA_VERSION}`);
      }
      const data = window.MARBLE_ASSEMBLY;
      if (!data) throw new Error("Assembly data missing");

      const chunkUrls = data.mesh_chunks;
      let loaded = 0;
      await Promise.all(
        chunkUrls.map(async (url) => {
          await loadScript(url);
          loaded++;
          setProgress(`Loading CAD Meshes (${Math.round((loaded / chunkUrls.length) * 100)}%)...`);
        })
      );
      for (const chunk of window.MARBLE_MESH_CHUNKS || []) registerChunk(chunk);
      window.MARBLE_MESH_CHUNKS = []; // release the base64 strings
      ...
      let meshCount = 0;
      robotRoot.traverse((o) => { if (o.isMesh) meshCount++; });
      window.__marbleViewer = { meshes: meshCount, geometries: geometryCache.size, frames: 0 };
  ...
  function animate() {
    ...
    if (window.__marbleViewer) window.__marbleViewer.frames++;
  }
